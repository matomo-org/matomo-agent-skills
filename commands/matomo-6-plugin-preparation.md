---
description: Prepare a separately distributed plugin's 6.x-dev branch for Matomo 6 — prep commit, CI and static-analysis updates, Vue rebuild, screenshot sync, verification
---
...
Use this command to prepare a separately distributed plugin's `6.x-dev` branch: the branch itself, the Matomo 6 prep commit, CI workflows, static-analysis configuration, the Vue build, UI screenshots, and the verification that proves the plugin still works against a Matomo 6 core.

This command is intentionally major-specific. It encodes facts of the Matomo 5 → 6 transition — the PHP 8.1 floor, PHPStan 2, the `matomo6_*` CI aliases, the Vite Vue build — so its branch names and version values are the transition's own rather than the shared tracked target dev branch behavior. The core team's `prepare6x` branches (`plugin-LogViewer`, `plugin-QueuedTracking`) are the reference implementations; check one before deviating from anything here.
...
4. The central CI pattern was in flux when this command was written. Establish what `matomo-org/github-action-tests` provides today, per `## CI State Check`, rather than assuming this command's snapshot.
...
Use this command when the task is one or more of:
...
5. Expect the suites to pass unchanged when the plugin needs no code migration. Failures referencing removed or changed core APIs are the signal that code compatibility work exists, which this command does not cover.

## Code Compatibility Is Not Covered

This command prepares the branch; it does not migrate code. The catalogue of Matomo 6 core breaking changes was still being produced when this command was written.
...
2. Once the breaking-changes catalogue exists, extend this command or add a companion for the code half, rather than growing ad hoc notes here.
