...
2. `matomo-code-change-quality-loop`
- Runs a focused self-review and fix loop after Matomo code is generated or modified.
- Reviews only files changed in the current task unless a scoped related edit is needed.
- Routes changed files through the applicable Matomo review gates for security, API development, plugin architecture, Twig, Vue, migrations, i18n, deprecation, documentation, code quality, and tests.
- Wraps existing review expectations rather than adding an independent rule set, so `matomo-review` routing does not need a new route for it.
- Use after creating, editing, or patching Matomo PHP, JavaScript, Vue, Twig, tests, migrations, plugin code, code-facing documentation, or related repository files.
3. `matomo-test-runner`
