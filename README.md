...
- Prepares a separately distributed plugin's `6.x-dev` branch: the Matomo 6 prep commit (version marker `6.0.0`, requirement range, changelog entry), update-file naming against the marker, CI workflow and static-analysis updates for the PHP 8.1 floor, the Vite rebuild of `vue/dist`, UI screenshot syncing from CI artifacts, and verification against a Matomo 6 core.
- Intentionally major-specific: it encodes facts of the Matomo 5 → 6 transition — PHPStan 2's hard failure on nonexistent `excludePaths` entries, the `matomo6_*` PHP aliases, the UI job's Node 24 and explicit database inputs — with the core team's `prepare6x` branches as the reference implementations, and checks the current state of `matomo-org/github-action-tests` before editing workflows because the central CI pattern was in flux when the skill was written.
- Knows what self-heals — the `minimum_required_matomo`/`maximum_supported_matomo` targets resolve from `plugin.json` at run time — changes only the workflow's pinned inputs that actually break (the PHP versions throughout, plus the UI job's Node and database inputs), and verifies the resolved target from the job log's `Testing against` line rather than trusting it.
