...
- Knows what self-heals — the `minimum_required_matomo`/`maximum_supported_matomo` targets resolve from `plugin.json` at run time — changes only the PHP pins that actually break, and verifies the resolved target from the job log's `Testing against` line rather than trusting it.
