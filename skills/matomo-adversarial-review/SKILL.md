...
- `Lens fan-out` lists the lenses, which run at full six-lens width regardless of the target and without `matomo-review`'s read budget. Depth per lens is what this skill is for, so a lens reads until its mandate is answered; that is the cost of the pass and the reason it is not the default review.
