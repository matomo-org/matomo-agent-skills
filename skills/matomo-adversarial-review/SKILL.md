...
This skill adds five things to a normal review: greater depth, a consistency-over-novelty stance, a stricter per-finding standard, the full written record, and the two mandates `matomo-review` does not carry. It defines no severity criteria of its own.
...
This skill is not the shipping gate and not a higher quality bar. `matomo-review` decides what is safe to put in a customer's hands, and since every merge is released, that is the only gate either skill has. This skill exists to learn from a change: to teach the developer something, to sharpen judgment about Matomo conventions, and to find what the regular review would have missed so the review skills themselves can improve.
...
2. Do not re-decide release readiness. Report the verdict `matomo-review` would reach on the same diff. If depth surfaced a genuine blocker the regular flow would have missed, the change still cannot go to customers, but record that as rule 4 requires.
...
`matomo-review` runs four lenses and reports only what carries a release consequence. Two mandates it deliberately does not carry are restored here, because this skill is read to learn rather than to gate, and a class with no shipping consequence is exactly what its added groups exist to hold. `matomo-review`'s `What no lens carries` states why the release gate drops them; none of that reasoning claims they are not worth knowing.

1. **Coverage adequacy** — whether the changed behavior carries the coverage `matomo-test-runner` expects, judged by reading the tests the diff adds against the behavior it changes. Load `matomo-test-runner`. A gap belongs in `Noted`, with a pointer to the lifecycle stage that owns it, and never in a severity bucket: a missing test reaches no customer, so it moves neither `Verdict` nor `Release readiness`.
2. **Naming, idiom, and layer precedent** — the precedent probe over every named artifact the diff introduces, plus framework idioms and plugin layer separation. Load `matomo-css-development-rules` and `matomo-frontend-direction` where the diff matches them. These are `Consistency` items carrying a `Judgment` and a `Priority`, not severities.

So the fan-out here is six lenses where `matomo-review` runs four. That is a real part of this pass's cost and another reason it is not the default review.

Where a restored observation is also a defect, rule 6 of `Consistency Over Novelty` governs and it is reported in the severity buckets as the defect it is. Restoring a mandate adds a class to look at; it does not create a route by which a non-shipping observation acquires a severity.
...
- `Mechanical checks` lists all nine by name in the `matomo-review` order, each with its result or `n/a — <reason>`, including the ones that found nothing. The checks themselves run unchanged: depth belongs to the search layer, and the decidable layer has no depth to add.
- `Lens fan-out` lists the lenses, which run at full six-lens width regardless of the target and without `matomo-review`'s read budget: its four, plus the two `Restored Mandates` adds back. Depth per lens is what this skill is for, so a lens reads until its mandate is answered; that is the cost of the pass and the reason it is not the default review.
...
- `Evidence probes` lists `Precedent probe`, `Untrusted-input inventory`, and `Scope attribution`, each with what it covered and what it returned, or `n/a — <reason>`. The precedent probe runs at full width here — every named artifact the diff introduces, not the two release-relevant surfaces `matomo-review` narrows it to — because `Consistency` is what the probe feeds and this skill restores that group. Give the inventory's rows, including each value's trust classification and the pre-processing size bound enforcing it; the rows are the teaching material.
...
7. Add a `Skill Gaps` section immediately before `Next Steps`. List every finding the regular review would not have surfaced, each with the skill that needs the rule, the rule or check that was missing, and whether the gap is release-relevant or teaching-only. Write `No gaps: every finding above is reachable from the existing rules.` when that is the case. This section is how adversarial passes pay back into the review skills.
8. In `Next Steps`, prioritize by release risk. Sequence `Conform` consistency items by their priority; do not list `Justified divergence` items as required work.
