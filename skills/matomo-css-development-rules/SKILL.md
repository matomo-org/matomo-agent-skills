...
1. Authoring or reviewing a `.less`/`.css` file next to a Vue component.
...
5. Adding or reviewing util classes.
6. Naming `@keyframes` animations.
7. Writing responsive breakpoints / media queries.
8. Using Less `calc()` or component-level variables.
...
- Exception: util classes (`u-`, section D) and app state classes (`app-`, on `html`/`body`,
  rules 15–16) are not component-prefixed.
...
5. The block root class is camelCase. Do not put a dash or underscore inside a name; a
   ...
   classes, rules 15–16).
- OK: `.evolutionBadge`
- OK (namespaced): `.mtm-notification`, `.u-textUppercase`, `.app-darkMode`
...
15. The root and elements may be styled according to `html`/`body` state classes. Use this
    for app-wide state such as a feature flag or dark mode.
- OK: `body.app-darkMode .evolutionBadge__number`

16. Name application state classes (set on `html`/`body`) with the `app-` namespace prefix;
    use `app-` for new app state classes.
- OK: `.app-darkMode`, `.app-featureFlagXyz`
...
- NOT OK: `.evolutionBadge__arrowTop.evolutionBadge__arrow--top`
...
- Exception: an `html`/`body` app state class may prefix a block/element selector
  (rules 15–16), e.g. `body.app-darkMode .evolutionBadge__number`.

19. No tag names in selectors. If an element has no class, add one.
- NOT OK: `p.evolutionBadge__number`
  → use `.evolutionBadge__number`
- Exception: the `body` (or `html`) tag may be used to force app state classes
  (rules 15–16), e.g. `body.app-darkMode`.
...
## E. Media queries (desktop-first)

27. This is a desktop-first project: write breakpoints with `@media (max-width: <width>)`
    only.
...
28. Use only multiples of 40 for media-query sizes. This groups nearby breakpoints and
    limits edge cases and existing display states.
- OK: `@media (max-width: 1200px) { … }`
- NOT OK: `@media (max-width: 768px) { … }`, `@media (max-width: 992px) { … }`

29. Do not use `@media (max-height: <height>)`. It is only valid for components used on
...
30. Wrap `calc()` in Less escaping so the browser (not the Less compiler) evaluates it.
- OK: `width: ~"calc(100% - 8px)";`
- NOT OK: `width: calc(100% - 8px);`

31. Component-level variables are defined inside the block definition and prefixed with an
    underscore.
```less
.block {
  @_padding: 8px;
...
2. Classes only, all prefixed with the component name in camelCase (except `u-` util and
   `app-` state classes); no tag/id/attribute (except `body`/`html` for app state classes).
...
8. At most two classes per selector, descendant combinator only, no chained classes
   (except an `app-` state class on `body`/`html`).
9. Prefer element modifiers over root-modifier descendant selectors.
10. Only state pseudo-classes / pseudo-elements; no structural pseudo-classes
    (`:has()` allowed only for `:focus`/`:focus-visible`).
11. Util classes use the `u-` prefix, are self-describing, single-class, and use
    `!important`; `!important` appears nowhere else.
12. Media queries are desktop-first: `max-width` only, sizes in multiples of 40; no
    `min-width`/`min-height`; `max-height` forbidden (no evidence it is needed yet).
13. Less `calc()` is escaped (`~"calc(...)"`); component variables are block-scoped and
    underscore-prefixed.
