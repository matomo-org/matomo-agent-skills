...
9. Give every lens the `Surface Inventory` rows its mandate owns, and require one verdict per row and question: `met` with the code that meets it, `unmet` as a finding, or `unreviewed` with the reason. A row is not a candidate a lens may decline to consider, because the enumeration already settled that it is in scope, and item 5's floor allowance covers reaching one. Each question takes a verdict and the row takes one of the same three words: a row `met` on one of its questions and `unmet` on another is `unmet`, because the questions are conjunctive. The ledger a lens returns carries exactly the rows its mandate owns, so two runs' ledgers are the same length and line up row for row; a surface the lens finds that the enumeration did not carry is a finding or a coverage row, not a row appended to the ledger.
...
2. `Mechanical` states that all nine checks ran and the inventory's row and `unmet` counts, then names by number only the checks that produced a finding, pointing at it: `9/9 ran, 11 surfaces, 2 unmet. #5 → finding #2.` Checks that were clean or `n/a` are covered by the count and are not listed, named, or explained. The row count is the number of rows in `<shared-diff>/surfaces.md`, and `unmet` counts those rows their owning lens returned `unmet` — not obligations, not questions, and nothing a lens recorded outside the ledger `First-Pass Lens Fan-Out` item 9 defines. Both counts are on this line rather than in a label of their own because the row list is owed identically across runs and comparing the count is the cheapest way for a reader to see whether it was; the `unmet` count is a tally of verdicts rather than something owed, and it is comparable at all only because it counts one thing.
