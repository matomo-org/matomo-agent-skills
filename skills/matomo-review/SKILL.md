...
11. Internal returns are records, not prose. A lens's coverage rows, a verifier's verdict, and the material behind the receipt are read by the merge and then discarded, so each one carries its claim and the evidence that establishes it and then stops: no restated mandate, no account of method, no summary of the work just done. What a context generates is the dearest thing it produces and is charged again as input to whoever reads it, which makes narration inside an internal return the one cost a review pays twice and gets nothing for.
12. Dispatch and then wait. Never poll a running context for progress or re-list the dispatched agents: a poll returns nothing the review can use and costs a full request against that same largest context. Completion arrives on its own.
...
3. Name in each lens's mandate the routed rule sets it applies, and let the lens load them. Two lenses that apply the same rule set read it twice, which costs a few thousand tokens and buys keeping the rule in the context that applies it.
...
5. A lens must not skip part of its mandate on the assumption that another lens covers it.
6. Each lens returns its findings with evidence, the routed rule sets it read, the coverage rows for the paths it applies to, and the basis for anything it calls sound, all in the record form Rule 11 requires. Everything but the findings feeds the internal ledger, the merge, and the receipt; none of it is review output.
