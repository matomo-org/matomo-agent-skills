...
- when the aliases cannot be resolved, report the tested range as unresolved rather than guessing. The floors from items 1 and 2 still govern syntax, so the plan proceeds against them; what is lost is the corroboration that the approach holds at the top of the tested range, and that goes into the plan's risks as unverified rather than into `Version constraints` as fact.
