...
- `git -C <repo> branch -r --list 'origin/*-dev'` lists the candidates when there is no usable upstream. Pick the one this change targets rather than the newest: a plugin remote commonly still carries `3.x-dev` and `4.x-dev` alongside the current one.
...
- do not substitute the remote's recorded default for this. `refs/remotes/origin/HEAD` records whatever the remote calls default, which can be `main` while the change targets `5.x-dev`, and a plain `git fetch` never refreshes it, so it is capable of being both off-target and stale. `git -C <repo> remote set-head origin -a` updates it if you want it as a cross-check, but the targeted `*-dev` branch governs.
- the resolved value is remote-qualified, such as `origin/5.x-dev`. Use it as it stands; prefixing it with `origin/` again produces a ref that does not exist.
